...
from hidden_server.controls import SocketButton, Devices
from .. import views_utls as utls


socket_button = Blueprint('socket_button', __name__, url_prefix='/socket_button')

sockets = Devices('socket_button')


@socket_button.route('/get_current_state', methods=['POST'])
def get_current_state():
	params = utls.get_input_params(request.data, ['serial_num'])
	serial_num = params['params']['serial_num']
	a_socket = SocketButton(serial_num)
	sockets.load_to_device(a_socket)
	return a_socket.get_current_state()


@socket_button.route('/get_current_states', methods=['POST'])
...
@socket_button.route('/get_all_states_in_range', methods=['POST'])
...
@socket_button.route('/last_request_time', methods=['POST'])
...
@socket_button.route('/enter_requested_action', methods=['POST'])
...
@socket_button.route('/last_requested_action', methods=['POST'])
...
@socket_button.route('/requested_actions_in_range', methods=['POST'])
...
@socket_button.route('/from_device', methods=['POST'])
def from_device():
	params = utls.get_input_params(request.data, ['serial_num', 'state', 'slave_server',
	'measurement_delay', 'conn_reist_trails', 'min_delay', 'num_delays'])
	serial_num = params['params']['serial_num']
...
@socket_button.route('/device_settings', methods=['POST'])
def device_settings():
	params = utls.get_input_params(request.data, ['serial_num'])
	serial_num = params['params']['serial_num']
	a_socket = SocketButton(serial_num)
